...
// import { generateFileName } from '../utils/file-renamer'; // Not used - user selects filename
...
let jobs: any[] = [];
let uploadSessions: Map<string, UploadSession> = new Map(); // Store upload sessions
let userCredits = 100;
...
router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    let documentsToProcess: { buffer: Buffer; pageNumber?: number; originalBuffer?: Buffer }[] = [];
    ...
    try {
      ...
      const availableFields: { key: string; value: string; displayName: string }[] = [];
      if (extractedFields) {
...
router.get('/download/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    if (!session || session.status !== 'completed') {
      res.status(404).json({ error: 'Session not found or not ready' });
      return;
    }
    ...
    session.processedFiles.forEach((file: any) => {
      if (file.pages) {
        // Multi-page document
        ...
        if (file.extractedData) {
          Object.entries(file.extractedData).forEach(([key, value]: [string, any]) => {
            if (value?.value !== null && value?.value !== undefined && key !== '_allFields') {
              rowData[key] = value.value;
            }
          });
...
router.post('/rename/:sessionId', async (req: Request, res: Response) => {
  try {
    const sessionId = req.params.sessionId;
